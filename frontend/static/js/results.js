document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderReport(data) {
    ...
    // סטריפ מידע כללי
    if(el.bhType) el.bhType.textContent = data.business_type || "—";
    if(el.bhArea) el.bhArea.textContent = data.area_sqm ?? "—";
    if(el.bhSeats) el.bhSeats.textContent = data.seating_capacity ?? "—";
    if(el.bhCity) el.bhCity.textContent = data.city ?? "—";
    ...
    if (rules.length > 0) {
      ...
      if(el.requirementsList) el.requirementsList.innerHTML = `<p>לא נמצאו חוקים</p>`;
      show(el.requirementsSection);
    }
  ...
  function renderCharts(grouped, rules) {
    if (!document.getElementById("rulesPie") || !document.getElementById("priorityBar")) return;
